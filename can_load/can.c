...
void can_config()
{
	...
	*/
	...
	CAN1->FM1R = 0;					// Two 32-bit registers Identifier Mask mode
	CAN1->FS1R = (1<<0) + (1<<1) + (1<<2) + (1<<3); // single 32-bit scale configuration
	
	CAN1->FFA1R = 0; 	// assign filters to FIFO0
	// filter 0
	CAN1->sFilterRegister[0].FR1 = MKU1_ADDR<<(8+3);	// identifier
	CAN1->sFilterRegister[0].FR2 = 0xff<<(8+3);			// mask
	// filter 1
	CAN1->sFilterRegister[1].FR1 = MKU2_ADDR<<(8+3);	// identifier
	CAN1->sFilterRegister[1].FR2 = 0xff<<(8+3);			// mask
	// filter 2
	CAN1->sFilterRegister[2].FR1 = MKU3_ADDR<<(8+3);	// identifier
	CAN1->sFilterRegister[2].FR2 = 0xff<<(8+3);			// mask	
	// filter 3
	CAN1->sFilterRegister[3].FR1 = MKU4_ADDR<<(8+3);	// identifier
	CAN1->sFilterRegister[3].FR2 = 0xff<<(8+3);			// mask		
	
	CAN1->FA1R = (1<<0) + (1<<1) + (1<<2) + (1<<3);		// bank 0,1,2,3 are active
	
	CAN1->FMR &= ~(1<<0);			// leave filter init mode
